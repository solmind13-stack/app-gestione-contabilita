// src/app/(app)/previsioni/revisione-entrate/page.tsx
'use client';
...
import type { IncomeForecastSuggestion, Movimento, PrevisioneEntrata } from '@/lib/types';
...
export default function RevisioneSuggerimentiEntratePage() {
    const { user, isUserLoading } = useUser();
    const firestore = useFirestore();
    const { toast } = useToast();
    const router = useRouter();

    const [selectedSuggestions, setSelectedSuggestions] = useState<IncomeForecastSuggestion[]>([]);
    ...
    const suggestionsQuery = useMemo(() => 
        firestore && user ? query(collection(firestore, 'users', user.uid, 'incomeForecastSuggestions'), where('status', '==', 'pending')) : null, 
    [firestore, user]);

    const movimentiQuery = useMemo(() => firestore ? collection(firestore, 'movements') : null, [firestore]);

    const { data: suggestions, isLoading: isLoadingSuggestions } = useCollection<IncomeForecastSuggestion>(suggestionsQuery);
    ...
    const handleSelectSuggestion = (suggestion: IncomeForecastSuggestion, checked: boolean) => {
        ...
        try {
            ...
            for (const pattern of selectedSuggestions) {
                ...
                const createForecast = (date: Date) => {
                    ...
                    const newForecastRef = doc(collection(firestore, 'incomeForecasts'));
                    const newForecast: Omit<PrevisioneEntrata, 'id'> = {
                        societa: pattern.societa as 'LNC' | 'STG',
                        anno: date.getFullYear(),
                        mese: formatFns(date, 'MMMM', { locale: it }),
                        dataPrevista: formatFns(date, 'yyyy-MM-dd'),
                        ...
                        iva: 0.22, // Default IVA, user can change later
                        certezza: 'Certa',
                        probabilita: 1.0,
                        stato: 'Da incassare',
                        createdBy: user.uid,
                ...
                const suggestionRef = doc(firestore, 'users', user.uid, 'incomeForecastSuggestions', pattern.id);
                batch.update(suggestionRef, { status: 'accepted' });
            }

            await batch.commit();
            toast({ title: 'Previsioni Create!', description: `${createdCount} nuove previsioni di entrata sono state aggiunte per il ${generationYear}.` });
    ...
    const handleRejectAll = async () => {
        ...
        try {
            ...
            suggestions.forEach(suggestion => {
                const suggestionRef = doc(firestore, 'users', user.uid, 'incomeForecastSuggestions', suggestion.id);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-between md:items-center">
                <div>
                  <h1 className="text-3xl font-bold">Revisione Suggerimenti Entrate</h1>
                  <p className="text-muted-foreground">
                    L'AI ha identificato questi pattern di incasso. Seleziona e crea le previsioni per l'anno desiderato.
                  </p>
                </div>
            </div>
                                                            ...
                                                            <TableBody>
                                                                {sourceMovements.map(mov => (
                                                                    <TableRow key={mov.id}>
                                                                        ...
                                                                        <TableCell className="text-right text-green-600">{formatCurrency(mov.entrata)}</TableCell>
                                                                    </TableRow>
                                                                ))}
                                                            </TableBody>
                                                        </Table>
