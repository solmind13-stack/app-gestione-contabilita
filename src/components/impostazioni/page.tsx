...
const CompanyFormSchema = z.object({
  ...
  sigla: z.string().min(1, 'La sigla è obbligatoria.').max(10, 'Massimo 10 caratteri.').transform(val => val.toUpperCase()),
  ...
  path: ['vatId'],
    ...
    if (name.includes('staygreen') && data.sigla !== 'STG') return false;
    if (name.includes('nuova costruzione') && data.sigla !== 'LNC') return false;
    return true;
}, {
    message: "La sigla suggerita per questa ragione sociale è 'STG' o 'LNC'.",
    path: ['sigla'],
});
